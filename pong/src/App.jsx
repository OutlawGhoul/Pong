...
const App = () => {
  ...
  const initialBallState = useMemo(
    () => ({ x: 289, y: 190, speedX: 4, speedY: 4 }),
    []
  );
  const initialPaddleState = useMemo(() => ({ left: 150, right: 150 }), []);
  ...
  const movePaddle = useCallback((side, speed) => {
    setPaddles((prevPaddles) => {
      let updatePaddles = { ...prevPaddles };
      if (side === "right") {
        updatePaddles.right = Math.max(
          Math.min(updatePaddles.right + speed, containerHeight - paddleHeight),
          0
        );
        paddlesRef.current.right = updatePaddles.right;
      } else if (side === "left") {
        updatePaddles.left = Math.max(
          Math.min(updatePaddles.left + speed, containerHeight - paddleHeight),
          0
        );
  ...
  const moveCPUPaddle = useCallback(
    (ball) => {
  ...
  }, [initialBallState, initialPaddleState]);
    ...
    setBall((prevBall) => {
      ...
      if (
        ...
        if (ballTop > paddleLeftTop && ballBottom < paddleLeftBottom) {
          ...
          ballTop <= paddleLeftTop + ballRadius ||
          ballBottom >= paddleLeftBottom - ballRadius
        ) {
      ...
      if (
        ...
        if (ballTop > paddleRightTop && ballBottom < paddleRightBottom) {
          ...
          ballTop <= paddleRightTop + ballRadius ||
          ballBottom >= paddleRightBottom - ballRadius
        ) {
          updateBall.speedY = -updateBall.speedY;
          updateBall.speedX = -updateBall.speedX;
        }
      ...
      if (
        Math.abs(updateBall.speedX) >= 6 ||
        Math.abs(updateBall.speedY) >= 6
      ) {
  ...
  }, [checkWinner, gameMode, gameRunning, moveCPUPaddle, resetGame]);
  ...
  }, [gameRunning, handleKeyPress, updateBallPosition]);
  ...
  }, [gameRunning, gameMode, ball, moveCPUPaddle]);
  ...
  }, [popupVisible, updateBallPosition, winnerPopupVisible]);
      ...
      <div className="pong-container" tabIndex="0">
        ...
        <div
          className={`paddle paddle-right ${
            gameMode === "cpu" && gameRunning ? "cpu-active" : "human-active"
          }`}
          id="paddle-right"
